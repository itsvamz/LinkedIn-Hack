...
import { Search, MessageSquare, Video, Mail, Phone, MapPin, User } from 'lucide-react';
import { getAllRecruiters } from '@/lib/api';
import { useToast } from '@/components/ui/use-toast';

// Define Recruiter interface
interface Recruiter {
  id: string | number;
  name?: string;
  company?: string;
...
const Recruiters = () => {
  ...
  const roles = ['Software Engineering', 'Product Management', 'Data Science', 'Designer', 'Marketing Manager'];
  ...
  const ProfileModal = ({ recruiter }: { recruiter: Recruiter }) => (
    <Dialog>
  ...
  const AvatarChatModal = ({ recruiter }: { recruiter: Recruiter }) => (
    <Dialog>
      <DialogTrigger asChild>
                  ...
                  <CardHeader className="text-center">
                    <Avatar className="w-20 h-20 mx-auto mb-4">
                      ...
                      <AvatarFallback className="bg-blue-600 text-white text-lg">
                        {(recruiter?.name || 'Unknown').split(' ').map(n => n?.[0] || '').join('').toUpperCase()}
                      </AvatarFallback>
                    </Avatar>
